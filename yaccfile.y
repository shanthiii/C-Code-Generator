...
%token START_PROCEDURE END_FUNCTION ENDDOWHILE FOR REPEAT ENDFOR FROM READ QUOTE WHILE_DO
%token<code> VAR NUM
%token NUMBER
%token<code> DATATYPE
...
V  :  VAR FROM NUM TO NUM{ if($3>=$5){printf("%s = %s ; %s >= %s ; %s --)",$1,$3,$1,$5,$1);}
	else{printf("%s = %s ; %s <= %s ; %s ++)",$1,$3,$1,$5,$1);}}
	;
...
PR : PRINT DATATYPE COMMA VAR {g=check($4); getspec($2); if(g==1)printf("printf(\"%s\",%s);\n",str,$4); else exit(0); }
   | PRINT{printf("printf(\"");} QUOTE N A
   ;

A : DATA N QUOTE{printf("\"%s);",name); strcpy(name,"");}
  | QUOTE{printf("\");");} 
  ;
